...
def draw_top_3(Top3Array, draw, name_dx, score_dx, dy, font, scrfont):
    ...
    for list in Top3Array:
        ...
        print('{0}위: {1} - {2}pt'.format(seq_no, list.get('userName'), score))
...
def draw_general(json_data, rank, draw, name_dx, score_dx, dy, font, scrfont):
    ...
    for i in json_data:
        if i['rank'] >= rank_specify:
            score = format(i.get('score'), ',')
            print('{0}위: {1} - {2}'.format(rank_specify, i.get('userName'), score + 'pt'))
            print('Template에 순위를 작성합니다')
...
def main(json_data):
    ...
    # Top 3
    print("Top 3")
    draw_top_3(json_data, draw, name_dx, score_dx, dy, font, scrfont)
